...
PREDICT_DEVICE = torch.device('cuda:3' if torch.cuda.is_available() else 'cpu')

def main():
    model_load = "/disk/yjt/PersonalTarGuess/model/pitg/t_collection_4kw_e_cos_m_pitg_host_v3.pt"
    model = PITGBeamSearchEvaluator(model_load, device=PREDICT_DEVICE)
    inputs = "/disk/data/targuess/3_query/Collection1_100k.csv"
    outputs = "/disk/yjt/PersonalTarGuess/result/csv/pitg/t_collection_4kw_q_Collection1_100k_m_pitg_host_v3.csv"
