...
class Pass2PathModel(nn.Module):
    ...
    def forward_train(self, pwds, edits):
        ...
        encoder_hidden = self.encoder(pwds, encoder_hidden)
    ...
    def forward_greedy_predict(self, pwds):
        batch_size = pwds.size(0)
        encoder_hidden = self.encoder.init_hidden(batch_size, self.device)
        encoder_hidden = self.encoder(pwds, encoder_hidden)
        decoder_input = self.decoder.init_start_token(batch_size, self.start_token, self.device)
    ...
    def forward_beamsearch(self, pwds, beamwidth=10, topk=10, max_predict=8):
        batch_size = pwds.size(0)
        encoder_hidden = self.encoder.init_hidden(batch_size, self.device)
        encoder_hidden = self.encoder(pwds, encoder_hidden)
    ...
    def forward(self, pwds, edits=None, beamwidth=10, topk=10):
        ...
        if self.mode == "train":
            ...
            with torch.no_grad():
                ans = self.forward_greedy_predict(pwds)
