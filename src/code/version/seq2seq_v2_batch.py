...
def split_tensors(tensor1, tensor2):
    # 拆分张量
    splits1 = torch.split(tensor1, 1, dim=0)
    splits2 = torch.split(tensor2[0], 1, dim=1)
    splits3 = torch.split(tensor2[1], 1, dim=1)

    # 将拆分结果转换为列表
    splits = [(split1, (split2, split3)) for split1, split2, split3 in zip(splits1, splits2, splits3)]
    return splits
...
class Pass2PathModel(nn.Module):
    ...
    def _search_batch_nodes(self, nodes):
        batch_size = len(nodes)
        decoder_input = torch.cat([node.wordid for node in nodes], dim=0)
        hc = torch.cat([node.h[0] for node in nodes], dim=1)
        hx = torch.cat([node.h[1] for node in nodes], dim=1)
        decoder_hidden = (hc, hx)
        decoder_output, decoder_hidden = self.decoder(decoder_input, decoder_hidden)
        decoder_output = decoder_output.squeeze(1)
        return split_tensors(decoder_output, decoder_hidden)
    
    def forward_beamsearch(self, pwds, x_length, beamwidth=10, topk=10, max_queue_size=20000, search_batch=64):
        batch_size = pwds.size(0)
        encoder_hidden = self.encoder.init_hidden(batch_size, self.device)
        encoder_hidden = self.encoder(pwds, encoder_hidden, x_length)
        maxlen = self.maxlen
        ...
        for t in range(batch_size):
            ...
            while nodes.qsize() <= max_queue_size and nodes.qsize() > 0:
                input_nodes = []
                while len(input_nodes) < search_batch and nodes.qsize() > 0:
                ...
                if len(end_nodes) >= topk:
                    break
                ans = self._search_batch_nodes(input_nodes)
                for idx, (decoder_output, decoder_hidden) in enumerate(ans):
                    topv, top_ids = decoder_output.topk(beamwidth, dim=1)
                    logps = -topv
                    for i in range(beamwidth):
                        decode_id = top_ids[:, i].unsqueeze(0)
                        logp = logps[0, i].item() + input_nodes[idx].logp
