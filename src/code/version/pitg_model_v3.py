...
class PITGModel(nn.Module):
    def __init__(self, vocab_size, e1_vocab_size, e2_vocab_size, trans_size, pad_token,start_token,end_token,hidden_size=128, 
                 embed_size=200, maxlen=32, num_layers=1,device=torch.device('cpu'), ratio="1:1"):
        '''
        ...
        self.decoder = LSTMDecoder(decode_size=self.output_size, hidden_size = hidden_size, num_layers=num_layers)
    ...
    def forward_beamsearch(self, pwds, seed1, seed2, beamwidth=10, topk=10, max_predict=8):
        batch_size = pwds.size(0)
