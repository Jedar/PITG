...
class PITGModel(nn.Module):
    ...
    def forward_beamsearch(self, pwds, seed1, seed2, beamwidth=10, topk=10, max_predict=6):
        ...
        for t in range(batch_size):
            ...
            if len(guesses) > topk:
                guesses = guesses[:topk]
            outputs[t] = guesses
        return outputs
    
    def forward_beamsearch_batch(self, pwds, seed1, seed2, beamwidth=10, topk=10, max_predict=6):
        '''
        Warning: beamwidth * max_predict > topk
        '''
        batch_size = pwds.size(0)
        ...
        encoder_hx = encoder_hidden[1]
        maxlen = self.maxlen
        outputs = [MaxHeap(topk) for _ in range(batch_size)]
