...
def init_cli():
    ...
    cli.add_argument("-n", dest="cnt", default=5000000, type=int)
    args = cli.parse_args()
    return {
        "input": args.input, 
        "save": args.save, 
...
def main():
    ...
    for email, data in tqdm.tqdm(pair_reader(data_path), total=args["cnt"]):
        ...
        d, dist = find_med_backtrace(pwd1, pwd2)
        dist = tuple(dist)
        counter1[region][dist] += 1
        counter2[hostname][dist] += 1
        counter3[d] += 1
    ...
    res = {
        "region": top_values(region_cnt, counter1, 100, 500),
        "host": top_values(host_cnt, counter2, 100, 500)
    }
    ...
    total = sum([x for x in counter3.values()])
    for d, value in counter3.items():
        print(f">>> {d}: {value/ total}")
